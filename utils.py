"""
All code here do not belong to me. Obtained from Yu-Qiao's repository
...
class AbstractSeq2SeqDataset(Dataset):
    def __init__(
        ...
        **dataset_kwargs
        ...
        print("SELF.SRC_LENS: ", len(self.src_lens))
...
def sortish_sampler_indices(data: List, bs: int, shuffle=True) -> np.array:
    ...
    # Changed permute to shuffle
    sort_idx = np.concatenate(random.sample(ck_idx[1:], len(ck_idx[1:]))) if len(ck_idx) > 1 else np.array([], dtype=np.int)
...
def post_tokenize(filepath):
    """
    Takes in a generated text file, and performs post-processing. 
    For example: 

        Mr. Potatohead's arm was found under the desk. 

        becomes: 

        mr . potatohead ' s arm was found under the desk . 
    """
    new_fpath = "".join(filepath.split(".untokenized"))
    
    with open(new_fpath, "w") as outfile, open(filepath, "r") as infile: 
        for hypothesis in infile.readlines():
            hypothesis = hypothesis.lstrip().rstrip()
            
            new_hyp = ' '.join(re.split('(\W)', hypothesis))
            new_hyp = ' '.join(new_hyp.split()).lower()

            outfile.write(new_hyp + "\n")
...
def base_arguments():
    ...
    # Yoinked all arguments from yu-qiao's repo
